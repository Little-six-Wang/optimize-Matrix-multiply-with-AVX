...
```
Based on `MMult_8x8_2.c`, the modification is mainly to put the 64 `AddDot` subroutines in `MMult_8x8_2.c` into the `AddDot4x4` subroutine, so that the subroutine calculates 64 inner products at one time.
