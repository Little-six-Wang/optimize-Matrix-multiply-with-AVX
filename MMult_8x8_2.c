...
void MY_MMult(int m, int n, int k, double* a, int lda, double* b, int ldb, double* c, int ldc) {
	int i, j;
	for (i = 0; i < n; i++) {	//Loop over the columns of C
		for (j = 0; j < m; j += 8) {	//Loop over the rows of C
			/* Update the C(i, j) with the inner product of the ith row of A and the jth 
			column of B */
